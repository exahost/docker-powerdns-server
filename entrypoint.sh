...
if $MYSQL_AUTOCONF ; then
  ...
  # Set up new env
  sed -r -i "s/^[# ]*master=.*/master=${MASTER_BOOL}/g" /etc/pdns/pdns.conf
  sed -r -i "s/^[# ]*slave=.*/slave=${SLAVE_BOOL}/g" /etc/pdns/pdns.conf
  sed -r -i "s/^[# ]*webserver-password=.*/webserver-password=${WEBSERVER_PASS}/g" /etc/pdns/pdns.conf
